...
def update_item(id):
    with get_db_connection() as conn:
        if request.method == 'POST':
            ...
            row = conn.execute("SELECT * FROM posts WHERE id = ?", (id,)).fetchone()
            categories = ["CARDING", "HACKING", "CRACKING", "LEAKS", "OFFTOPIC"]
            return render_template('update_item.html', post=dict(row), categories=categories)
...
def admin_members():
    with get_db_connection() as conn:
        users = conn.execute("SELECT id, username, email, role, status, ip_address, user_agent FROM users ORDER BY id").fetchall()
